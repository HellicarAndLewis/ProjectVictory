...
class DeckLink {
 public:
  ...
  int getWidth(BMDDisplayMode mode); /* get the width for the given display mode on the initialized card. it will return < 0 on error, make sure to call setup() first.*/
  int getHeight(BMDDisplayMode mode); /* get the height for the given display mode. */
 private:
...
// returns < 0  on error
inline int DeckLink::getWidth(BMDDisplayMode mode) {
  ...
  return card->getWidth(mode);
}

// returns < 0  on error
inline int DeckLink::getHeight(BMDDisplayMode mode) {
